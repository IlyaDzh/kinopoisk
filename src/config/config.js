export const API_KEY = '3ac9e9c4b5b41ada30de1c0b1e488050';
//разделить компоненты
//добавить папку для страниц
//импортировать все из index.js в компонентах
//разделить scc на отдельные scss
//https://www.youtube.com/watch?v=NMxSlH2RobA

export const GENRES = [
    { value: "28", label: "Боевик" },
    { value: "12", label: "Приключения" },
    { value: "16", label: "Мультфильм" },
    { value: "35", label: "Комедия" },
    { value: "80", label: "Криминал" },
    { value: "99", label: "Документальный" },
    { value: "18", label: "Драма" },
    { value: "10751", label: "Семейный" },
    { value: "14", label: "Фэнтези" },
    { value: "36", label: "История" },
    { value: "27", label: "Ужасы" },
    { value: "10402", label: "Музыка" },
    { value: "9648", label: "Детектив" },
    { value: "10749", label: "Мелодрама" },
    { value: "878", label: "Фантастика" },
    { value: "10770", label: "Телевизионный фильм" },
    { value: "53", label: "Триллер" },
    { value: "10752", label: "Военный" },
    { value: "37", label: "Вестерн" }
]

export const COUNTRIES = [
    { value: "ru", label: "Россия" },
    { value: "en", label: "США" },
    { value: "pt", label: "Бразилия" },
    { value: "hu", label: "Венгрия" },
    { value: "de", label: "Германия" },
    { value: "cn", label: "Гонконг" },
    { value: "da", label: "Дания" },
    { value: "te", label: "Индия" },
    { value: "id", label: "Индонезия" },
    { value: "ga", label: "Ирландия" },
    { value: "es", label: "Испания" },
    { value: "it", label: "Италия" },
    { value: "zh", label: "Китай" },
    { value: "nl", label: "Нидерланды" },
    { value: "no", label: "Норвегия" },
    { value: "ko", label: "Южная Корея" },
    { value: "fr", label: "Франция" },
    { value: "fi", label: "Финляндия" },
    { value: "sv", label: "Швеция" },
    { value: "ja", label: "Япония" }
]